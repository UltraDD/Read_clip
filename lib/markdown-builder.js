 ...
 */
(function () {
  // YAML 安全字符串：碰到冒号/反引号/换行/引号 → 加引号转义
  function yamlString(s) {
    if (s === null || s === undefined) return '""';
    const str = String(s);
    if (!str) return '""';
    // 简单字符串走字面量
    if (/^[\w\u4e00-\u9fa5\-_.]+$/.test(str) && str.length < 200) {
      // 但仍要避免歧义关键字（true/false/yes/no/null/数字）
      if (/^(true|false|yes|no|null|on|off)$/i.test(str)) return `"${str}"`;

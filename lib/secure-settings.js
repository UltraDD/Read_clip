...
(function () {
  ...
  function normalizeSettings(s) {
    s = s || {};
    return {
      github_owner: (s.github_owner ?? DEFAULTS.github_owner).trim(),
      github_repo: (s.github_repo ?? DEFAULTS.github_repo).trim(),
      github_branch: (s.github_branch ?? DEFAULTS.github_branch).trim(),
      github_token: (s.github_token || '').trim(),
      github_path_article: (s.github_path_article ?? DEFAULTS.github_path_article).trim(),
      github_path_media: (s.github_path_media ?? DEFAULTS.github_path_media).trim(),
      github_path_pdf: (s.github_path_pdf ?? DEFAULTS.github_path_pdf).trim(),

      deepseek_api_key: (s.deepseek_api_key || '').trim(),
      deepseek_model: (s.deepseek_model ?? DEFAULTS.deepseek_model).trim(),

      oss_region: (s.oss_region || '').trim(),
      oss_bucket: (s.oss_bucket || '').trim(),
      oss_ak: (s.oss_ak || '').trim(),
      oss_sk: (s.oss_sk || '').trim(),
  ...
  function emptyDraft() {
    // 给 manager 页面用的"空配置默认值"，token / oss / deepseek 均为空
      ...
      if (remember.passphrase) {
        try {
          ...
          if (encrypted) {
            ...
            const normalized = normalizeSettings(plain);
  ...
  async function save(passphrase, settingsObj, rememberMe = false) {
    ...
    const normalized = normalizeSettings(settingsObj);
    validateSettings(normalized);
